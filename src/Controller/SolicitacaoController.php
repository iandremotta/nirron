    ...
    public function aprovar(Request $request, Solicitacao $solicitacao, ConfiguracoesRepository $configuracoesRepository, SolicitacaoRepository $solicitacaoRepository): Response
    {
        if ($this->isCsrfTokenValid('aprovar' . $solicitacao->getId(), $request->request->get('_token'))) {
            ...
            if (in_array('ROLE_ADMINISTRADOR_ASSESSORIA', $this->getUser()->getRoles()) || in_array('ROLE_ADMINISTRADOR_LOGISTICA', $this->getUser()->getRoles()) || in_array('ROLE_SUPER', $this->getUser()->getRoles())) {
                ...
                $pdf = new Fpdi();
                $file = $this->getParameter('kernel.project_dir') . '\public\images\products\\' . $solicitacao->getImageName();
                ...
                for ($i = 1; $i <= $pagecount; $i++) {
                    ...
                    $pdf->Write(0, 'Aprovador: ' . $solicitacao->getAdministrador()->getNome());
                }
                ...
                try {
                    ...
                    $filesystem->chmod($this->getParameter('kernel.project_dir') . '\public\images\products\\', 0700, 0000, true);

                    copy($this->getParameter('kernel.project_dir') . '\public\images\products\\' . $solicitacao->getImageName(), $configuracoes->getPath() . '\\' . $result[0] . '\\'  . $result[1] . '\\'  . $result[2] . '\\' . $solicitacao->getImageName());
